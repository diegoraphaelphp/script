...
//			require_once("../lib/log.php");
		...
		case "incluirUsuarios":
			...
			$nome   = antInjection($_POST["nome"]);
			$und    = antInjection($_POST["unidade"]);
			$mun    = antInjection($_POST["municipio"]);
			...
			if ($existe){
				...
				$sql.= "USU_Login, USU_Senha, USU_Status, USU_Paginacao, USU_Email, USU_Telefone, USU_NFE) VALUES ('".$und."', '".$mun."', ";
		...
		case "alterarUsuarios":
			...
			if ($existe){
				...
				$sql.= "USU_Paginacao = '".$pagina."', USU_NFE = '".$nfe."' WHERE USU_IDUsuario = ".$idU;
				$qry = $banco->executarQuery($sql);
...
//			 require_once("../lib/log.php");
		...
		case "importandoSigater":
			...
			require_once("../class/ConexaoMySqlI.php");

			$bancoMySqlI = ConexaoMySqlI::singleton();
			
			//	pasta onde os arquivos serão salvos
			$localArqSalvos = '../tmp/';
			//$arqComErros = array();
			$arquivosCadastro = $arquivosEntidade = $arquivosRDE = $arqComErros = array();
			
			//	mostra como o arquivo de cadastro é identificado
			$identificacaoArqCadastro = array("_cad");
			$identificacaoArqEntidade = array("_ent");
			$identificacaoArqRDE = array("_rde", "_rde_grupo");
			...
			//chdir($localArqSalvos);

			$bancoMySqlI->desativarAutoCommit();
			$sucesso = true;
			$contArquivos = 0;
				...
				if ($handle){
					...
					while (!feof($handle)) {
                        ...
                        if ($valida[1] == "cad.txt"){
    						...
    						$conjapelido		= $campo[39];
    						$conjnomepai		= $campo[40];
    						$conjnomemae	    = $campo[41];
    						$nomeproprietario   = $campo[42];
    						...
    						if($bancoMySqlI->existe($sql))	{
    							$sqlC.= "UPDATE tb_cadastro SET
    								...
    								RGEMISSOR = ".avaliaArgParaSql($rgemissor).",
    								...
    								CONJRGEMISSOR = ".avaliaArgParaSql($conjrgemissor).",
    								...
    								CONJAPELIDO = ".avaliaArgParaSql($conjapelido).",
    							...
    							$sqlC .= "INSERT INTO tb_cadastro
    								...
    								".avaliaArgParaSql($idusuario, 0).", ".avaliaArgParaSql($propriedade).", ".avaliaArgParaSql($sexo).", ".avaliaArgParaSql($rendapecuaria).",
    								".avaliaArgParaSql($rgemissor).", ".avaliaArgParaSql($rgdata).", ".avaliaArgParaSql($naturalidade).",
    								".avaliaArgParaSql($nomepai).", ".avaliaArgParaSql($nomemae).",
    								".avaliaArgParaSql($conjnatural).", ".avaliaArgParaSql($conjrgemissor).", ".avaliaArgParaSql($conjrgdata).",
    								".avaliaArgParaSql($nascimento).", ".avaliaArgParaSql($conjnascimento).", ".avaliaArgParaSql($conjapelido).",
                            ...
                            $bancoMySqlI->executarQuery($sqlC);
                            $bancoMySqlI->commit();
                            ...
                            $sqlC.= "VALUES (".substr($arquivo, 8, 4).", ".intval($projeto).", ".intval($atividade).", ".substr($arquivo, 6, 2).", ";
						...
						try{
						      //if (!empty($sqlC)) {
						          //confirma inserções/alterações feitas para esse arquivo
						      //}
						}catch(Exception $e)	{
							//	desfaz inserções/alterações feitas para esse arquivo
							$bancoMySqlI->rollback();
							$arqComErros[] = $arquivo;
			...
			//	tratamento da mensagem dos arquivos com erro
			if ($arqComErros)	{
				//	retira arquivos repetidos
				$arqComErros = array_unique($arqComErros);
                
				$msg .= "Arquivos importados com sucesso: ".count($contents)." <br>";
				$msg .= "Arquivos não importados (com erro): ".count($arqComErros)." <br>";			
				$msg .= "Nome dos arquivos com erros: ";
				foreach($arqComErros as $arq)	{
					$msg .= $arq.'<br>';
				}

			}else{
				//$msg .= "Arquivo(s) importado(s) com sucesso.";
				$msg .= "Arquivo(s) importado(s) com sucesso: ".count($contents)." <br>";
				$msg .= "Arquivos não importados (com erro): 0";			
				$retorno = 1;
			}
