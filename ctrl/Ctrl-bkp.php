	...
	//		require_once("../lib/log.php");
		...
		case "gerarTXT":
			...
			if ($tab == "A"){
				...
				$sqlA  = "SELECT * FROM tb_planoanual ORDER BY PLA_IDAnual";
				$arq   = "planoanual";
			}elseif ($tab == "S"){
				...
				$sqlA  = "SELECT * FROM tb_usuarios ORDER BY USU_IDUsuario";
				$arq   = "usuarios";				
			}elseif ($tab == "W"){
				...
				if ($handle){
					...
					while (!feof($handle)) {
                        ...
                        if ($valida[1] == "cad.txt"){
    						...
    						$idcadastro	  	    = $campo[0];
    						$idmunicipio		= $campo[1];
    						$nome				= addslashes($campo[2]);
    						$endereco		    = addslashes($campo[3]);
    						$cpf				= $campo[4];
    						$rg				    = $campo[5];
    						$data		 		= ($campo[6]) ? inverterFormatoData($campo[6], '/', '-') : null;
    						$estadocivel		= $campo[7];
    						$conjuge			= $campo[8];
    						$cpfconjuge		    = $campo[9];
    						$rgconjuge		    = $campo[10];
    						$rendagricola		= $campo[11];
    						$individuos		    = $campo[12];
    						$grauinstprod		= $campo[13];
    						$grauinstconj		= $campo[14];
    						$tppropriedade	    = $campo[15];
    						$area				= $campo[16];
    						$apelido			= $campo[17];
    						$identidade		    = $campo[18];
    						$titposse			= $campo[19];
    						...
    						$captacao			= $campo[21];
    						$estrada			= $campo[22];
    						$eletricidade		= $campo[23];
    						$distancia			= $campo[24];
    						$idusuario			= $campo[25];
    						$propriedade		= addslashes($campo[26]);
    						$sexo				= $campo[27];
    						$rendapecuaria	    = $campo[28];
    						$rgemissor		    = $campo[29];
    						$rgdata		 	    = ($campo[30]) ? inverterFormatoData($campo[30], '/', '-') : null;
    						$naturalidade		= addslashes($campo[31]);
    						$nomepai			= addslashes($campo[32]);
    						$nomemae		    = addslashes($campo[33]);
    						$conjnatural		= $campo[34];
    						$conjrgemissor	    = $campo[35];
    						$conjrgdata 		= ($campo[36]) ? inverterFormatoData($campo[36], '/', '-') : null;
    						$nascimento 		= ($campo[37]) ? inverterFormatoData($campo[37], '/', '-') : null;
    						$conjnascimento	    = ($campo[38]) ? inverterFormatoData($campo[38], '/', '-') : null;
    						$conjapelido		= addslashes($campo[39]);
    						$conjnomepai		= addslashes($campo[40]);
    						$conjnomemae	    = addslashes($campo[41]);
    						$nomeproprietario   = addslashes($campo[42]);
    						$cpfproprietario	= $campo[43];
    						$respfamilia		= $campo[44];
    						$latitude			= $campo[45];
    						$longitude			= $campo[46];
    						$numportdef		    = $campo[47];
    						$categoria		    = $campo[48];
    						...
    						if($banco->existe($sql))	{
    							$sqlC.= "UPDATE tb_cadastro SET
    								IDMUNICIPIO = ".avaliaArgParaSql($idmunicipio, 0).",
    								NOME = ".avaliaArgParaSql($nome).",
    								ENDERECO = ".avaliaArgParaSql($endereco).",
    								RG = ".avaliaArgParaSql($rg).",
    								DATA = ".avaliaArgParaSql($data).",
    								ESTADOCIVEL = ".avaliaArgParaSql($estadocivel).",
    								CONJUGE = ".avaliaArgParaSql($conjuge).",
    								CPFCONJUGE = ".avaliaArgParaSql($cpfconjuge).",
    								RGCONJUGE = ".avaliaArgParaSql($rgconjuge).",
    								RENDAGRICOLA = ".avaliaArgParaSql($rendagricola, 0).",
    								INDIVIDUOS = ".avaliaArgParaSql($individuos, 0).",
    								GRAUINSTPROD = ".avaliaArgParaSql($grauinstprod).",
    								GRAUINSTCONJ = ".avaliaArgParaSql($grauinstconj).",
    								TPPROPIEDADE = ".avaliaArgParaSql($tppropriedade).",
    								AREA = ".avaliaArgParaSql($area, 0).",
    								APELIDO = ".avaliaArgParaSql($apelido).",
    								IDENTIDADE = ".avaliaArgParaSql($identidade, 0).",
    								TITPOSSE = ".avaliaArgParaSql($titposse).",
    								AGUA = ".avaliaArgParaSql($agua).",
    								CAPTACAO = ".avaliaArgParaSql($captacao).",
    								ESTRADA = ".avaliaArgParaSql($estrada).",
    								ELETRICIDADE = ".avaliaArgParaSql($eletricidade).",
    								DISTANCIA = ".avaliaArgParaSql($distancia, 0).",
    								IDUSUARIO = ".avaliaArgParaSql($idusuario, 0).",
    								PROPRIEDADE = ".avaliaArgParaSql($propriedade).",
    								SEXO = ".avaliaArgParaSql($sexo).",
    								RENDAPECUARIA = '".avaliaArgParaSql($rendapecuaria, 0)."',
    								RGEMISSOR = '".avaliaArgParaSql($rgemissor, 0)."',
    								RGDATA = ".avaliaArgParaSql($rgdata).",
    								NATURALIDADE = ".avaliaArgParaSql($naturalidade).",
    								NOMEPAI = ".avaliaArgParaSql($nomepai).",
    								NOMEMAE = ".avaliaArgParaSql($nomemae).",
    								CONJNATURAL = ".avaliaArgParaSql($conjnatural).",
    								CONJRGEMISSOR = '".avaliaArgParaSql($conjrgemissor, 0)."',
    								CONJRGDATA = ".avaliaArgParaSql($conjrgdata).",
    								NASCIMENTO = ".avaliaArgParaSql($nascimento).",
    								CONJNASCIMENTO = ".avaliaArgParaSql($conjnascimento).",
    								CONJAPELIDO = '".avaliaArgParaSql($conjapelido, 0)."',
    								CONJNOMEPAI = ".avaliaArgParaSql($conjnomepai).",
    								CONJNOMEMAE = ".avaliaArgParaSql($conjnomemae).",
    								NOMEPROPRIETARIO = ".avaliaArgParaSql($nomeproprietario).",
    								CPFPROPRIETARIO = ".avaliaArgParaSql($cpfproprietario).",
    								RESPFAMILIA = ".avaliaArgParaSql($respfamilia).",
    								LATITUDE = ".avaliaArgParaSql($latitude).",
    								LONGITUDE = ".avaliaArgParaSql($longitude).",
    								NUMPORTDEF = '".avaliaArgParaSql($numportdef, 0)."' ,
    								CATEGORIA = ".avaliaArgParaSql($categoria)."
    								WHERE CPF = '$cpf';";
    						}else{                                                    
    							$sqlC .= "INSERT INTO tb_cadastro
    								(IDMUNICIPIO, NOME, ENDERECO, 
    								CPF, RG, DATA, 
    								ESTADOCIVEL, CONJUGE, CPFCONJUGE, 
    								RGCONJUGE, RENDAGRICOLA, INDIVIDUOS,
    								GRAUINSTPROD, GRAUINSTCONJ, TPPROPIEDADE, AREA, 
    								APELIDO, IDENTIDADE, TITPOSSE, AGUA, 
    								CAPTACAO, ESTRADA, ELETRICIDADE, DISTANCIA, 
    								IDUSUARIO, PROPRIEDADE, SEXO, RENDAPECUARIA, 
    								RGEMISSOR, RGDATA, NATURALIDADE, 
    								NOMEPAI, NOMEMAE, 
    								CONJNATURAL, CONJRGEMISSOR, CONJRGDATA, 
    								NASCIMENTO, CONJNASCIMENTO, CONJAPELIDO, 
    								CONJNOMEPAI, CONJNOMEMAE, 
    								NOMEPROPRIETARIO, CPFPROPRIETARIO, RESPFAMILIA, 
    								LATITUDE, LONGITUDE, 
    								NUMPORTDEF, CATEGORIA)
    								VALUES
    								(".avaliaArgParaSql($idmunicipio, 0).", ".avaliaArgParaSql($nome).", ".avaliaArgParaSql($endereco).",
    								".avaliaArgParaSql($cpf).", ".avaliaArgParaSql($rg).", ".avaliaArgParaSql($data).",
    								".avaliaArgParaSql($estadocivel).", ".avaliaArgParaSql($conjuge, 1, 1).", ".avaliaArgParaSql($cpfconjuge).", 
    								".avaliaArgParaSql($rgconjuge).", ".avaliaArgParaSql($rendagricola, 0).", ".avaliaArgParaSql($individuos, 0).", 
    								".avaliaArgParaSql($grauinstprod).", ".avaliaArgParaSql($grauinstconj).", ".avaliaArgParaSql($tppropriedade).", ".avaliaArgParaSql($area).",
    								".avaliaArgParaSql($apelido).", ".avaliaArgParaSql($identidade, 0).", ".avaliaArgParaSql($titposse).", ".avaliaArgParaSql($agua).",
    								".avaliaArgParaSql($captacao).", ".avaliaArgParaSql($estrada).", ".avaliaArgParaSql($eletricidade).", ".avaliaArgParaSql($distancia, 0).",
    								".avaliaArgParaSql($idusuario, 0).", ".avaliaArgParaSql($propriedade).", ".avaliaArgParaSql($sexo).", '".avaliaArgParaSql($rendapecuaria, 0)."',
    								'".avaliaArgParaSql($rgemissor, 0)."', ".avaliaArgParaSql($rgdata).", ".avaliaArgParaSql($naturalidade).",
    								".avaliaArgParaSql($nomepai).", ".avaliaArgParaSql($nomemae).",
    								".avaliaArgParaSql($conjnatural).", '".avaliaArgParaSql($conjrgemissor, 0)."', ".avaliaArgParaSql($conjrgdata).",
    								".avaliaArgParaSql($nascimento).", ".avaliaArgParaSql($conjnascimento).", '".avaliaArgParaSql($conjapelido, 0)."',
    								".avaliaArgParaSql($conjnomepai).", ".avaliaArgParaSql($conjnomemae).",
    								".avaliaArgParaSql($nomeproprietario).", ".avaliaArgParaSql($cpfproprietario).", ".avaliaArgParaSql($respfamilia).",
    								".avaliaArgParaSql($latitude).", ".avaliaArgParaSql($longitude).",
    								'".avaliaArgParaSql($numportdef, 0)."', ".avaliaArgParaSql($categoria).");";
    						}

                            $banco->executarQuery($sqlC);
//                            $bancoMySqlI->commit();
                            ...
                            $idmun = substr($arquivo, 0, 3);
                            
                            //VERIFICA SE REGISTRO JÁ EXISTE...
                            $sqlV = "SELECT PLE_IDExecucao FROM tb_plano_execucao WHERE MUN_IDMunicipio = '".intval($idmun)."' ";
                            $sqlV.= "AND PLE_Ano = '".substr($arquivo, 8, 4)."' AND PRJ_IDProjeto = '".intval($projeto)."' ";
                            ...
                            if ($rowM[0]["IDCADASTRO"] > 0) $sqlV.= " AND FAM_IDFamilia = '".intval($rowM[0]["IDCADASTRO"])."'";
                            ...
                            if (count($rowV) == "0"){
                        		...
                        		$mes  = substr($data, 5, 2);
                        		$dia  = substr($data, 8, 2);
                                $sema = semana_do_ano($dia, $mes, $ano);
                                ...
                                $sqlC.= "VALUES (".substr($arquivo, 8, 4).", ".intval($projeto).", ".intval($atividade).", '".$sema."', ";
                                $sqlC.= intval($quantidade).", '".$data."', ".intval($usuario).", '".$row8[0]["MUN_IDMunicipio"]."', ".intval($rowM[0]["IDCADASTRO"]).", ";
                                $sqlC.= intval($familias).", ".intval($indicador).", '".$origem."', '".$desc."');";
                                $banco->executarQuery($sqlC);
                            }
                            ...
                            if (count($rowV) == "0"){
                                ...
                                $sqlC.= "VALUES (".substr($arquivo, 8, 4).", ".intval($projeto).", ".intval($atividade).", ".$sema.", ";
                                $sqlC.= intval($quantidade).", '".$data."', ".intval($usuario).", ".intval($idmun).", ".intval($rowM[0]["IDCADASTRO"]).", ";
                                $sqlC.= intval($familias).", ".intval($indicador).", '".$origem."');";
                                $banco->executarQuery($sqlC);
...
//                            pa($campo); exit;
                            ...
                            $sqlV = "SELECT IDVEICULO FROM tb_veiculos WHERE PLACA = '".trim($campo[0])."'";
                            $rowV = $banco->listarArray($sqlV);
                            if (count($rowV) == "0"){
                                ...
                                $sqlC.= "VALUES ('".intval(antInjection($campo[8]))."', '".antInjection($campo[0])."', '".antInjection($campo[1])."', '".antInjection($campo[2])."', '".trim($campo[3])."', ";
                                $sqlC.= "'".antInjection($campo[4])."', '".antInjection($campo[5])."', 'A', '".antInjection($campo[6])."', '".antInjection($campo[7])."')";
                                
                            }else{
                                
                                $sqlC = "UPDATE tb_veiculos SET IDMUNICIPIO = '".intval(antInjection($campo[8]))."', DESCRICAO = '".antInjection($campo[1])."', ";
                            ...
                            $sqlC.= "VALUES ('".antInjection($campo[1])."', '".antInjection($conv->conData($campo[2]))."', '".antInjection(intval($campo[3]))."', ";
...
//                            exit($sqlC);
    						...
    						$municipio		= addslashes(trim($campo[0]));
    						$razaoSocial	= addslashes(trim($campo[1]));
    						$cnpj			= trim($campo[2]);
    						$endereco	    = addslashes(trim($campo[3]));
    						$bairro		    = addslashes(trim($campo[4]));
    						$uf				= trim($campo[5]);
    						$cep			= trim($campo[6]);
    						$telefone		= trim($campo[7]);
    						$responsavel	= addslashes(trim($campo[8]));
    						$cpf			= trim($campo[9]);
                            ...
                            $sqlC.= "VALUES (".intval($municipio).", '".$razaoSocial."', '".$cnpj."', '".$endereco."', '".$bairro."', '".$uf."', ";
                            $sqlC.= "'".$cep."', '".$telefone."', '".$responsavel."', '".$cpf."', '".$data."', ".substr($arquivo, 3, 3).", ";
                            $sqlC.= "'".$paa."', NOW());";
                            $banco->executarQuery($sqlC);
//                            $banco->commit();
...
//                        echo $sqlC.'<br><br>';
			...
			$msg = "Arquivo(s) importado(s) com sucesso: <b>".$contArquivos." </b><br>";
			$msg.= "Arquivos não importados (COM ERRO): <b>".$arqComErros."</b>";			
			$retorno = 1;

			goto2("../lib/Fachada.php?acao=".base64_encode("importarSigater")."&retorno=".$retorno."&msg=".$msg."&mod=".$_POST["mod"]);
