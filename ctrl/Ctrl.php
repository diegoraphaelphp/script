	...
	//		require_once("../lib/log.php");
		...
		case "gerarTXT":
			...
			if ($tab == "A"){
				...
				foreach($rowA as $a){
					$txt = completarComZero($a["MUN_IDMunicipio"])." ".addEspaco($a["MUN_Codigo"], 7)." ".addEspaco($a["MUN_Descricao"])." ".completarComZero($a["RDE_ID"])." ".$a["MUN_Status"]." ".completarComZero($a["REG_ID"]).CRLF;
					fwrite($abrir, $txt);				
				}
				...
				foreach($rowA as $a){
					...
					$txt = completarComZero($a["PLA_IDAnual"])." ".completarComZero($a["PLA_Ano"], 4)." ".completarComZero($a["PRJ_IDProjeto"])." ".completarComZero($a["ATV_IDAtividade"])." ".addEspaco($a["ATV_Prevqtd"], 20)." ".completarComZero($a["ATV_Prevfam"], 10)." ".completarComZero($a["USU_IDUsuario"])." ".completarComZero($a["MUN_IDMunicipio"])." ".$a["PLA_Data"].CRLF;
					fwrite($abrir, $txt);
				}
			}elseif ($tab == "S"){
				...
				foreach($rowA as $a){
					...
					$txt = completarComZero($a["USU_IDUsuario"])." ".completarComZero($a["MUN_IDMunicipio"])." ".addEspaco($a["USU_Nome"], 100)." ".addEspaco($a["USU_Login"], 50)." ".addEspaco($a["USU_Senha"], 32)." ".$a["USU_Status"].CRLF;
					fwrite($abrir, $txt);
				}				
			}elseif ($tab == "W"){
				...
				foreach($rowA as $a){
                    ...
                    //$a["RESPFAMILIA"] = trim($a["RESPFAMILIA"]);
                    //if (empty($a["RESPFAMILIA"])) 
                    if (empty($a["RENDAGRICOLA"])) $a["RENDAGRICOLA"] = "0";
                    if (empty($a["RENDAPECUARIA"])) $a["RENDAPECUARIA"] = "0";                    
                    if (empty($a["GRAUINSTPROD"])) $a["GRAUINSTPROD"] = "0";
                    if (empty($a["GRAUINSTCONJ"])) $a["GRAUINSTCONJ"] = "0";
                    if (empty($a["TPPROPIEDADE"])) $a["TPPROPIEDADE"] = "0";
                    if (empty($a["TIPORENDA"])) $a["TIPORENDA"] = "0";
                    if (empty($a["AREA"])) $a["AREA"] = "0";
                    if (empty($a["IDENTIDADE"])) $a["IDENTIDADE"] = "0";                    
                    if (empty($a["TITPOSSE"])) $a["TITPOSSE"] = "0";
                    if (empty($a["AGUA"])) $a["AGUA"] = "0";                    
                    if (empty($a["CAPTACAO"])) $a["CAPTACAO"] = "0";
                    if (empty($a["ESTRADA"])) $a["ESTRADA"] = "0";
                    if (empty($a["ELETRICIDADE"])) $a["ELETRICIDADE"] = "0";
                    if (empty($a["DISTANCIA"])) $a["DISTANCIA"] = "0";
                    if (empty($a["IDUSUARIO"])) $a["IDUSUARIO"] = "0";
                    if (empty($a["RENDAPECUARIA"])) $a["RENDAPECUARIA"] = "0";
                    if (empty($a["RGEMISSOR"])) $a["RGEMISSOR"] = "0";
                    if (empty($a["RGDATA"])) $a["RGDATA"] = "0000-00-00";                    
                    if (empty($a["CONJRGEMISSOR"])) $a["CONJRGEMISSOR"] = "0";
                    if (empty($a["CONJRGDATA"])) $a["CONJRGDATA"] = "0000-00-00";
                    if (empty($a["NASCIMENTO"])) $a["NASCIMENTO"] = "0000-00-00";
                    if (empty($a["CONJNASCIMENTO"])) $a["CONJNASCIMENTO"] = "0000-00-00";
                    if (empty($a["LATITUDE"])) $a["LATITUDE"] = "0";
                    if (empty($a["LONGITUDE"])) $a["LONGITUDE"] = "0";
                    if (empty($a["NUMPORTDEF"])) $a["NUMPORTDEF"] = "0";
                    if (empty($a["STATUS"])) $a["STATUS"] = "0";
                    if (empty($a["CATEGORIA"])) $a["CATEGORIA"] = "0";

					$txt = completarComZero($a["IDCADASTRO"])." ".completarComZero($a["IDMUNICIPIO"])." ".addEspaco($a["NOME"], 100)." ".addEspaco($a["ENDERECO"], 100)." ".addEspaco($a["CPF"], 11)." ".addEspaco($a["RG"], 20)." ".addEspaco($a["DATA"], 10)." ".addEspaco($a["ESTADOCIVEL"], 1)." ".addEspaco($a["CONJUGE"], 100)." ".addEspaco($a["CPFCONJUGE"], 11)." ".addEspaco($a["RGCONJUGE"], 20)." ".addEspaco($a["APELIDO"], 30)." ".addEspaco($a["PROPRIEDADE"], 100)." ".addEspaco($a["SEXO"], 1)." ".addEspaco($a["NATURALIDADE"], 30)." ".addEspaco($a["NOMEPAI"], 100)." ".addEspaco($a["NOMEMAE"], 100)." ".addEspaco($a["CONJNATURAL"], 30)." ".addEspaco($a["CONJAPELIDO"], 30)." ".addEspaco($a["CONJNOMEPAI"], 100)." ".addEspaco($a["CONJNOMEMAE"], 100)." ".addEspaco($a["NOMEPROPRIETARIO"], 100)." ".addEspaco($a["CPFPROPRIETARIO"], 14)." ".$a["RESPFAMILIA"]." ".addEspaco($a["RENDAGRICOLA"], 12)." ".completarComZero($a["INDIVIDUOS"])." ".$a["GRAUINSTPROD"]." ".$a["GRAUINSTCONJ"]." ".$a["TPPROPIEDADE"]." ".addEspaco($a["AREA"], 10)." ".$a["TIPORENDA"]." ".completarComZero($a["DEPENDENTES"])." ".addEspaco($a["IDENTIDADE"], 11)." ".addEspaco($a["TITPOSSE"], 1)." ".addEspaco($a["AGUA"], 1)." ".addEspaco($a["CAPTACAO"], 1)." ".addEspaco($a["ESTRADA"], 1)." ".addEspaco($a["ELETRICIDADE"], 1)." ".addEspaco($a["DISTANCIA"], 10)." ".completarComZero($a["IDUSUARIO"], 11)." ".addEspaco($a["RENDAPECUARIA"], 12)." ".addEspaco($a["RGEMISSOR"], 12)." ".addEspaco($a["RGDATA"], 10)." ".addEspaco($a["CONJRGEMISSOR"], 12)." ".addEspaco($a["CONJRGDATA"], 10)." ".addEspaco($a["NASCIMENTO"], 10)." ".addEspaco($a["CONJNASCIMENTO"], 10)." ".addEspaco($a["LATITUDE"], 6)." ".addEspaco($a["LONGITUDE"], 6)." ".addEspaco($a["NUMPORTDEF"], 6)." ".addEspaco($a["STATUS"], 1)." ".addEspaco($a["CATEGORIA"], 1).CRLF;                    
                    /* CRLF; */
					fwrite($abrir, $txt);
				}
		...
		case "validaLogin":		
			...
			$login = antInjection($_REQUEST["login"]);
			$senha = antInjection($_REQUEST["senha"]);
			...
			if (count($row) == "0"){			
              ...
              if (isset($_SESSION["sQUERY_STRING"])){

                $redir = $_SESSION["sQUERY_STRING"];
                unset($_SESSION["sQUERY_STRING"]);
                
                $vai = "../lib/Fachada.php?".$redir;
              }elseif ($senha == "123456"){
                $vai = "../lib/Fachada.php?acao=".base64_encode("frmAlterarSenha")."&mod=".base64_encode(5);
              }else{
                $vai = "../lib/Fachada.php?acao=".base64_encode("MenuPrincipal");
              }
              goto2($vai);
		...
		case "incluirUsuarios":
			...
			$login    = antInjection(strtolower($_POST["login"]));
...
//			require_once("../lib/log.php");
		...
		case "alterarAtividades":
            ...
            goto2("../lib/Fachada.php?acao=".base64_encode("cadastrarAtividades")."&alt=0&idp=".base64_encode($idp)."&mod=".$_POST["mod"]);
		...
		case "incluirElaboracao":
            ...
            $tec   = antInjection($_POST["tec"]);
            $media = antInjection($_POST["media"]);
			...
			goto2("../lib/Fachada.php?acao=".base64_encode("cadastrarElaboracao")."&mod=".$_GET["mod"]);
		...
		case "excluirElaboracao":
            ...
            $sql = "SELECT ATV_IDAtividade, MUN_IDMunicipio, PLA_Ano FROM tb_planoanual WHERE PLA_IDAnual = '".base64_decode($_GET["id"])."'";
            $row = $banco->listarArray($sql);
            if (count($row)>0){
                
                $sql2 = "DELETE FROM tb_planoanual WHERE MUN_IDMunicipio = '".$row[0]["MUN_IDMunicipio"]."' AND PLA_Ano = '".$row[0]["PLA_Ano"]."' ";
                $sql2.= "AND ATV_IDAtividade = '".$row[0]["ATV_IDAtividade"]."'";
...
//			exit($sql5);			
		...
		case "incluirOriProj":
			...
			for($i=0;$i<count($_POST["apl"]);$i++){
				...
				$sqlE   = "SELECT * FROM tb_orientacoes_x_atividades WHERE ATV_ID = ".$id." AND ORI_ID = ".$nome;
				$existe = $banco->existe($sqlE);
				if (!$existe){
					$sql = "INSERT INTO tb_orientacoes_x_atividades (ATV_ID, ORI_ID) VALUES (".$nome.", '".$id."')";
                    $qry = $banco->executarQuery($sql);
				}
			}
		...
		case "excluirOriProj":
			...
			for($i=0;$i<count($_POST["cod"]);$i++){
				...
				//excluindo...
				$sql = "DELETE FROM tb_orientacoes_x_atividades WHERE ORI_ID = ".$id[0]." AND ATV_ID = ".$id[1];
				$qry = $banco->executarQuery($sql);
			}
...
//			 require_once("../lib/log.php");
		...
		case "importandoSigater":
            ...
            ini_set("session.gc_max.lifetime", "21600");
            session_start();
			...
			foreach ($contents as $arquivo)	{
              @chmod($diretorio.$arquivo, 0777);
			  $handle = @fopen($diretorio.$arquivo, "r");
              $l = 0;
              if ($handle){
                
                $tam     = strlen($arquivo) - 7;
                $nomearq = substr($arquivo, $tam, strlen($arquivo));
				...
				while (!feof($handle)){
                    ...
                    if ($nomearq == "ent.txt"){
                        ...
                        $usuario        = antInjection(intval(substr($arquivo, 3, 5)));
              ...
              if ($handle){
				...
				while (!feof($handle)){
                    ...
                    if ($nomearq == "cad.txt"){
                        ...
                        $sql = "SELECT * FROM tb_cadastro WHERE CPF = '".antInjection($cpf)."'";                                                	
                        if($banco->existe($sql)){
                            ...
                            if ($data == "--") $data = date("Y-m-d"); else $data = "'".$data."'";
                        ...
                        $sqlLOF = "INSERT INTO tb_log_files (USU_ID, LOF_DataHora, MUN_ID, LOF_Arquivo, LOF_Linha, LOF_Status) ";
                        $sqlLOF.= "VALUES ('".$_SESSION["sIDUSUARIO"]."', '".date("Y-m-d H:i:s")."', '".$idmunicipio."', '".$arquivo."', '".$l."', '".$s."')";
                        //if ($s == "I") echo $sqlC.'<br><br>'.$sqlLOF.'<br><br>';
                        $qryLOF = $banco->execQRY($sqlLOF);
                    }
            ...
            //LISTANDO OS MOVIMENTOS DOS VEICULOS...
			foreach ($contents as $arquivo)	{
              @chmod($diretorio.$arquivo, 0777);
			  $handle = @fopen($diretorio.$arquivo, "r");
              $l = 0;
              ...
              if ($handle){
				...
				while (!feof($handle)){
                    ...
                    if ($nomearq == "rde.txt"){
                        ...
                        $quantidade = str_replace(",", ".", $campo[7]);
						//exit($quantidade.' teste ');
                        ...
                        $rowM = $banco->listarArray($sqlM);
                        if (empty($rowM[0]["IDCADASTRO"])) $rowM[0]["IDCADASTRO"] = 0;
                        
                        $idsem = semana_do_ano($data[0], $data[1], $data[2]);
                        
                        //VERIFICA SE REGISTRO JÁ EXISTE...
                        $sqlV = "SELECT PLE_IDExecucao FROM tb_plano_execucao WHERE MUN_IDMunicipio = '".$idmunicipio."' ";
                        $sqlV.= "AND PLE_Ano = '".$data[2]."' AND PRJ_IDProjeto = '".$projeto."' AND ATV_IDAtividade = '".$atividade."' ";
                        $sqlV.= "AND PLE_Semana = '".$idsem."' AND USU_IDUsuario = '".$usuario."' ";
                        ...
                        if (count($rowV) == "0"){
                            ...
                            $sqlC.= "VALUES ('".$data[2]."', '".$projeto."', '".$atividade."', '".$idsem."', ";
                            $sqlC.= "'".$quantidade."', '".$data[2]."-".$data[1]."-".$data[0]." 00:00:00', '".$usuario."', '".$idmunicipio."', '".$rowM[0]["IDCADASTRO"]."', ";
                            $sqlC.= "'".$familias."', '".$indicador."', '0');";
                        }else{	

                            $sqlC = "UPDATE tb_plano_execucao SET PLE_Qtd = '".$quantidade."' WHERE PLE_IDExecucao = '".$rowV[0]["PLE_IDExecucao"]."'";
              ...
              if ($handle){
				...
				while (!feof($handle)){
                    ...
                    if ($nomearq == "gru.txt"){
                        ...
                        //VERIFICA SE REGISTRO JÁ EXISTE...
                        $sqlV = "SELECT PLE_IDExecucao FROM tb_plano_execucao WHERE MUN_IDMunicipio = '".intval($idmunicipio)."' ";
                        $sqlV.= "AND PLE_Ano = '".$data[2]."' AND PRJ_IDProjeto = '".intval($projeto)."' ";
                        $sqlV.= "AND ATV_IDAtividade = '".intval($atividade)."' AND PLE_Semana = '".$idsem."' ";
            ...
            goto2("../lib/Fachada.php?acao=".base64_encode("MenuPrincipal")."&files=".$contArquivos."&mod=".$_REQUEST["mod"]);
...
*/						        
        ...
        case "atualizaQTDExecucao":
            ...
            $sql = "SELECT PLE_IDExecucao, PLE_Familias FROM tb_plano_execucao WHERE PLE_Ano = '".$r[1]."' AND PRJ_IDProjeto = '".$r[0]."' ";
            $sql.= "AND ATV_IDAtividade = '".$r[2]."' AND FAM_IDFamilia = '0' AND MUN_IDMunicipio = '".$r[3]."' ";
            ...
            if (count($row) == "0"){
                ...
                $sql2.= "VALUES ('".$r[1]."', '".$r[0]."', '".$r[2]."', '0', '0', '".date("Y-m-d H:i:s")."', '".$_SESSION["sIDUSUARIO"]."', '".$r[3]."', '0', '".$_GET["qtd"]."', ";
                $sql2.= "'0', '', '', '0')";
                $qry2 = $banco->executarQuery($sql2);

            }else{

                if ($row[0]["PLE_Familias"] != $_GET["qtd"]){

                    $sql2 = "UPDATE tb_plano_execucao SET PLE_Familias = '".$_GET["qtd"]."', USU_IDUsuario = '".$_SESSION["sIDUSUARIO"]."', PLE_Data = '".date("Y-m-d H:i:s")."' ";
                    $sql2.= "WHERE PLE_IDExecucao = '".$row[0]["PLE_IDExecucao"]."'";
                    $qry2 = $banco->executarQuery($sql2);
        ...
        case "atualizaQTDExecucao2":
            ...
            //VERIFICA SE EXISTE (INCLUSÃO / ATUALIZAÇÃO)...
            $sql = "SELECT PLE_IDExecucao, PLE_Qtd2 FROM tb_plano_execucao WHERE PLE_Ano = '".$r[1]."' AND PRJ_IDProjeto = '".$r[0]."' ";
            $sql.= "AND ATV_IDAtividade = '".$r[2]."' AND FAM_IDFamilia = '0' AND MUN_IDMunicipio = '".$r[3]."' ";
            $sql.= "AND ORI_IDOrientacao = '0'";
            $row = $banco->listarArray($sql);
            if (count($row) == "0"){
                ...
                $sql2 = "INSERT INTO tb_plano_execucao (PLE_Ano, PRJ_IDProjeto, ATV_IDAtividade, PLE_Semana, PLE_Qtd, PLE_Data, USU_IDUsuario, ";
                $sql2.= "MUN_IDMunicipio, FAM_IDFamilia, PLE_Familias, ORI_IDOrientacao, PLE_Desc_Atv, PLE_Origem, PLE_Qtd2) ";
                $sql2.= "VALUES ('".$r[1]."', '".$r[0]."', '".$r[2]."', '0', '0', '".date("Y-m-d H:i:s")."', '".$_SESSION["sIDUSUARIO"]."', '".$r[3]."', '0', '0', ";
                $sql2.= "'0', '', '', '".$qtd."')";
                $qry2 = $banco->executarQuery($sql2);

            }else{
                if ($row[0]["PLE_Qtd2"] != $qtd){
                    $sql2 = "UPDATE tb_plano_execucao SET PLE_Qtd2 = '".$qtd."', FAM_IDFamilia = '0', USU_IDUsuario = '".$_SESSION["sIDUSUARIO"]."', PLE_Data = '".date("Y-m-d H:i:s")."' ";
                    $sql2.= "WHERE PLE_IDExecucao = '".$row[0]["PLE_IDExecucao"]."'";
                    $qry2 = $banco->executarQuery($sql2);    
                }
