...
var popUpCal = {
		...
		this.keyDownFunction = function(e) {
			if (popUpCal.popUpShowing) {
				if (e.keyCode == 9) { // hide on tab out
					...
					jQuery('#calendar_clear').click();
				}
				else if (e.keyCode == 36 && e.ctrlKey) { // current on ctrl+home
					jQuery('#calendar_current').click();
				}
	...
	connectCalendar: function(target) {
		...
		if (this.autoPopUp == 'button' || this.autoPopUp == 'both') { // pop-up calendar when button clicked
			...
			jQuery((this.buttonImageOnly ? 'img' : 'button') + '.calendar_trigger', $input.parent('span')).click(this.showFunction);
		}
	...
	showCalendar: function() {
		...
		this.setupDayLinks();
		// clear button link
		jQuery('#calendar_clear').click(function() {
			popUpCal.clearDate();
		});
		// close button link
		jQuery('#calendar_close').click(function() {
			popUpCal.hideCalendar(popUpCal.speed);
		});
		...
		jQuery('#calendar_current').click(function() {
		...
		jQuery('#calendar_newMonth').change(function() {
			popUpCal.selectedMonth = this.options[this.selectedIndex].value - 0;
			popUpCal.adjustDate(); 
		});
		jQuery('#calendar_newYear').change(function() {
